...
def main():
    ...
    m = re.search(r'<header[^>]*class="site-header"[^>]*>.*?</header>',
                  body, re.DOTALL)
    assert m, "site-header block not found in home HTML"
    header_block = m.group(0)

    # Collect every anchor href in the header
    hrefs = re.findall(r'<a[^>]+href="([^"]+)"', header_block)
    if not hrefs:
        print("  (no nav links found — suspicious)")
        sys.exit(2)

    # Collect every id on the home page
    ids_on_page = set(re.findall(r'\bid="([^"]+)"', body))
